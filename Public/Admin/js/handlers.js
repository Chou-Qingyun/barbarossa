...
function uploadSuccess(file, serverData){
	addImage(serverData,file.name);
	var $svalue=$('form>input[name=document_path]').val();
	if($svalue==''){
		$('form>input[name=document_path]').val(serverData);
	}else{
		$('form>input[name=document_path]').val($svalue+"|"+serverData);
	}
...
function addImage(src,name){
    ...
    /*var newElement = "<li><div style=\"width:100px;height:50px;text-align: center;display:inline;\">" + name + "</div><img class='button' src="+window.path+"/images/fancy_close.png></li>";*/
...
//    var fid = $(this).parent().prevAll().length + 1;
    ...
    $.ajax({
        ...
        url: window.url+"/myCustomDocumentUploadDelete", //调用WebService的地址和方法名称组合---WsURL/方法名
        data: "src=" + src,
        success: function(data){
		var $val=$svalue.replace(data,'');
			$('form>input[name=document_path]').val($val);
        }
    });
    $(this).parent().remove();
}
