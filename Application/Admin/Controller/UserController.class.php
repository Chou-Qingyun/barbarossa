...
class UserController extends AdminController {
    ...
    public function submitPassword(){
        ...
        $res    =   $Api->updateInfo(I('post.id'), $password, $data);
        if($res['status']){
            $this->success('修改密码成功！');
        }else{
            $this->error($res['info']);
    ...
    public function submitEdit(){
        ...
        $auth_group_id = I('post.auth_group_id');
        $mobile = I('post.mobile');
        empty($mobile) && $this->error('手机号不能为空');
        $title = I('post.title');
        empty($title) && $this->error('职称不能为空');
        $custom_limit = I('post.custom_limit');
        empty($custom_limit) && $this->error('客户容量不能为空');
        $email = I('post.email');
        empty($email) && $this->error('邮箱不能为空');
        $uid = I('post.uid');
        empty($uid) && $this->error('参数错误,uid不能为空');
        $data = array('sex'=>$sex, 'department_id'=>$department_id, 'auth_group_id'=>$auth_group_id, 'mobile'=>$mobile, 
                'title'=>$title, 'custom_limit'=>$custom_limit, 'email'=>$email, 'uid'=>$uid,);
